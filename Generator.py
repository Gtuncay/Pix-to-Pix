...
class Block(nn.Module):
    def __init__(self, input_channels, output_channels, down = True, act = "relu", use_dropout = False):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, ksize, stride = 2, padding = 1, bias = False)
            if down
            else nn.ConvTranspose2d(input_channels, output_channels, ksize, 2, 1, bias = False),
...
class Generator(nn.Module):
    def __init__(self, input_channels = 1, features = 64): 
        super().__init__()
        self.initial_down = nn.Sequential(
            nn.Conv2d(input_channels, features, ksize, 2, 1),
            nn.LeakyReLU(0.2),
        ) #128
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(features*8, features*8, ksize, 2, 1), #1x1
            nn.ReLU(),
        )
        
        self.up1 = Block(features*8, features*8, down = False, act = "relu", use_dropout = True)
        self.up2 = nn.Sequential(
            nn.ConvTranspose2d(features*8*2, features*8, kernel_size = 4, stride = 2, padding = 1),
            nn.ReLU(),
        )
...
'''def test():
    ...
    #mach_number = torch.randn(16)
    #angle_attack = torch.randn(16)
    model = Generator(input_channels=1, features=64)
    preds = model(x) 
    print(preds.shape)
    #print(mark_vector.shape)
